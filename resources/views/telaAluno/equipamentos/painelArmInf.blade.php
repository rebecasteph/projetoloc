
<div class="grid-itens ">

    <a href="#">
        <img src="{!!asset('img\heroi\Heroi_1_Becky\arm-inf\item_1\icon\Becky 10.png')!!}" class=" item-img" alt="">
        <div class="tags has-addons justify-content-center">
            <span class="tag is-dark">PB</span>
            <span class="tag is-warning">333</span>
        </div>
    </a>
    <a href="#">
        <img src="{!!asset('img/icons/arm-inf.png')!!}" class="img-escura item-img" alt="">
        <div class="tags has-addons justify-content-center">
            <span class="tag is-dark">PB</span>
            <span class="tag">333</span>
        </div>
    </a>



</div>

